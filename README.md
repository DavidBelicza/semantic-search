...
| Config | `.json`, `.xml`, `.yaml`, `.yml`, `.ini`, `.properties` | Split by key path, so each chunk is titled with the setting it holds; comments are kept and credential values are redacted |
